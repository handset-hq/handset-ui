...
/**
 * Handset proxy routes for Express.
 *
 * Your Handset API key stays on the server; the Handset UI components in the
 * browser call these routes instead. Only the endpoints the components need
 * are exposed, and every request is scoped to the tenant you resolve from
 * your own session below.
 *
 * Mount at the path your components are configured for (default /api/handset):
 *
 *   import { handsetRoutes } from "./handset-routes";
 *   app.use("/api/handset", handsetRoutes());
 *
 * Works with Express 4 and 5 (Node 18+, global fetch).
 ...
 * BOUNDARY — wire it to your session (Passport, express-session, your own
 * cookie…).
 *
 * Return a tenant id (tnt_…) to scope reads to that tenant, or null if your
 * account is single-tenant. Throw { status, message } to reject.
 */
async function resolveTenantId(_req: Request): Promise<string | null> {
  // TODO: replace with your session lookup, e.g.:
  //   if (!req.session?.user) throw { status: 401, message: "Sign in first" };
  //   return req.session.user.handsetTenantId;
  return null;
}
...
export function handsetRoutes(): Router {
  const router = Router();
  // Keep POST bodies as raw text so they forward byte-for-byte. If your app
  // already runs express.json() globally, the object body is re-serialized
  // below instead.
  ...
  router.use(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const r = rejection as { status?: number; message?: string };
      res.status(r.status ?? 401).json({
        error: { code: "unauthorized", message: r.message ?? "Could not resolve tenant" },
      });
      return;
    }
